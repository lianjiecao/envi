...
struct FlowBlock
{
   FlowBlock(const NetflowV5Record &nfr, double &begin, double &end)
   {
      srcaddr = nfr.srcaddr; 
      dstaddr = nfr.dstaddr; 
      srcport = ntohs(nfr.srcport); 
      dstport = ntohs(nfr.dstport); 
      flowbegin = begin;
      flowend = end;
      npkts = ntohl(nfr.npkts);
      noctets = ntohl(nfr.noctets);
      proto = nfr.proto;
                   ...
                   int &tottcpflows, double &min_begin)
{
    ...
    for (int i = 0; i < ntohs(fh.flow_count); ++i)
    {
        ...
        // Modified version - Lianjie
        struct fttime ftt_begin = ftltime(ntohl(fh.sys_uptime), ntohl(fh.unix_secs), ntohl(fh.unix_nsecs), ntohl(fr.flowbegin));
        struct fttime ftt_end = ftltime(ntohl(fh.sys_uptime), ntohl(fh.unix_secs), ntohl(fh.unix_nsecs), ntohl(fr.flowend));
        double begin = double(ftt_begin.secs) + double(ftt_begin.msecs) / 1000.0;
        double end = double(ftt_end.secs) + double(ftt_end.msecs) / 1000.0;
...
//        struct in_addr ina1;
//        struct in_addr ina2;
//        ina1.s_addr = fb.srcaddr;
//        ina2.s_addr = fb.dstaddr;
//        std::cout << inet_ntoa(ina1) << ' ' << inet_ntoa(ina2) << ' ' << fb.flowbegin << ' ' << fb.flowend << fb.srcport << ' ' << fb.dstport << ' ' << int(fb.proto) << ' ' << fb.npkts << ' ' << fb.noctets << std::endl;
